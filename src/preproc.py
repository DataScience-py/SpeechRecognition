...
def GreedyDecoder(
    output, labels, label_lengths, blank_label=34, collapse_repeated=True
):
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        path = self.data_path_audio + self.data.iloc[idx, 0]  # type: ignore str + str
        sentence = self.data.iloc[idx, 1]
        sentence = normalization_text(sentence)  # type: ignore str
